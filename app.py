from flask import Flask, render_template, request, redirect
import os
import sqlite3
import psycopg2
from datetime import datetime

app = Flask(__name__)

# ===============================
# CONEXION
# ===============================
def get_conn():
    db_url = os.environ.get("DATABASE_URL")

    if db_url:
        return psycopg2.connect(db_url)
    else:
        return sqlite3.connect("datos.db")

def conectar():
    return get_conn()

# ===============================
# ACTUALIZAR TABLAS (ALTER)
# ===============================
def actualizar_tablas():
    con = conectar()
    cur = con.cursor()

    try:
        cur.execute("ALTER TABLE pagos ADD COLUMN saldado INTEGER DEFAULT 0")
    except:
        pass

    try:
        cur.execute("ALTER TABLE gastos ADD COLUMN saldado INTEGER DEFAULT 0")
    except:
        pass

    con.commit()
    con.close()

# ===============================
# CREAR TABLAS
# ===============================
def crear_tablas():
    con = conectar()
    cur = con.cursor()

    cur.execute("""
    CREATE TABLE IF NOT EXISTS clientes (
        id SERIAL PRIMARY KEY,
        nombre TEXT,
        abono REAL
    )
    """)

    cur.execute("""
    CREATE TABLE IF NOT EXISTS pagos (
        id SERIAL PRIMARY KEY,
        cliente_id INTEGER,
        fecha TEXT,
        importe REAL,
        persona TEXT,
        saldado INTEGER DEFAULT 0
    )
    """)

    cur.execute("""
    CREATE TABLE IF NOT EXISTS gastos (
        id SERIAL PRIMARY KEY,
        descripcion TEXT,
        fecha TEXT,
        importe REAL,
        persona TEXT,
        saldado INTEGER DEFAULT 0
    )
    """)

    con.commit()
    con.close()

# ejecutar al iniciar
crear_tablas()
actualizar_tablas()

# ===============================
# HOME
# ===============================
@app.route("/", methods=["GET"])
def index():
    con = conectar()
    cur = con.cursor()

    mes = request.args.get("mes")

    filtro = ""
    params = ()

    if mes:
        filtro = "WHERE SUBSTRING(fecha,1,7) = %s"
        params = (mes,)

    # ingresos
    cur.execute(f"""
        SELECT persona, SUM(importe)
        FROM pagos
        where saldado = 0
        { "AND SUBSTRING(fecha,1,7) = %s" if mes else "" }
        GROUP BY persona
        """, (mes,) if mes else ())
    
    ingresos = {row[0]: row[1] for row in cur.fetchall()}

    # gastos
    cur.execute(f"""
        SELECT persona, SUM(importe)
        FROM gastos
        where saldado = 0
        { "AND SUBSTRING(fecha,1,7) = %s" if mes else "" }
        GROUP BY persona
        """, (mes,) if mes else ())
    
    gastos = {row[0]: row[1] for row in cur.fetchall()}

    abel_ing = ingresos.get("Abel", 0)
    vale_ing = ingresos.get("Valeria", 0)

    abel_gas = gastos.get("Abel", 0)
    vale_gas = gastos.get("Valeria", 0)

    neto_abel = abel_ing - abel_gas
    neto_vale = vale_ing - vale_gas

    total = neto_abel + neto_vale
    mitad = total / 2 if total else 0

    balance = neto_abel - mitad

    if balance > 0:
        mensaje = f"Abel debe transferir a Valeria: ${round(balance,2)}"
    elif balance < 0:
        mensaje = f"Valeria debe transferir a Abel: ${round(abs(balance),2)}"
    else:
        mensaje = "Cuentas saldadas"

    con.close()

    return render_template("index.html",
        abel_ing=abel_ing,
        vale_ing=vale_ing,
        abel_gas=abel_gas,
        vale_gas=vale_gas,
        mensaje=mensaje,
        mes=mes
    )

# ===============================
# CLIENTES
# ===============================
@app.route("/clientes", methods=["GET", "POST"])
def clientes():
    con = conectar()
    cur = con.cursor()

    if request.method == "POST":
        nombre = request.form["nombre"]
        abono = request.form["abono"]

        cur.execute("INSERT INTO clientes (nombre, abono) VALUES (%s, %s)", (nombre, abono))
        con.commit()

    cur.execute("SELECT * FROM clientes")
    clientes = cur.fetchall()

    con.close()
    return render_template("clientes.html", clientes=clientes)

# ===============================
# COBROS
# ===============================
@app.route("/cobros", methods=["GET", "POST"])
def pagos():
    con = conectar()
    cur = con.cursor()

    if request.method == "POST":
        cliente_id = request.form["cliente_id"]
        importe = request.form["importe"]
        fecha = request.form["fecha"]
        persona = request.form["persona"]

        cur.execute("""
        INSERT INTO pagos (cliente_id, fecha, importe, persona)
        VALUES (%s, %s, %s, %s)
        """, (cliente_id, fecha, importe, persona))
        con.commit()

    cur.execute("""
    SELECT pagos.id, clientes.nombre, pagos.fecha, pagos.importe, pagos.persona
    FROM pagos
    JOIN clientes ON clientes.id = pagos.cliente_id
    ORDER BY pagos.fecha DESC
    """)

    pagos = cur.fetchall()

    cur.execute("SELECT * FROM clientes")
    clientes = cur.fetchall()

    con.close()
    return render_template("cobros.html", pagos=pagos, clientes=clientes)

# ===============================
# GASTOS
# ===============================
@app.route("/gastos", methods=["GET", "POST"])
def gastos():
    con = conectar()
    cur = con.cursor()

    if request.method == "POST":
        descripcion = request.form["descripcion"]
        fecha = request.form["fecha"]
        importe = request.form["importe"]
        persona = request.form["persona"]

        cur.execute("""
        INSERT INTO gastos (descripcion, fecha, importe, persona)
        VALUES (%s, %s, %s, %s)
        """, (descripcion, fecha, importe, persona))
        con.commit()

    cur.execute("SELECT * FROM gastos ORDER BY fecha DESC")
    gastos = cur.fetchall()

    con.close()
    return render_template("gastos.html", gastos=gastos)

# ===============================
# INFORME
# ===============================
@app.route("/informe")
def informe():
    con = conectar()
    cur = con.cursor()

    modo = request.args.get("modo", "pendiente")

    filtro = ""
    if modo == "pendiente":
        filtro = "WHERE saldado = 0"

    cur.execute(f"""
    SELECT fecha, clientes.nombre, importe, 'COBRO', persona
    FROM pagos
    JOIN clientes ON clientes.id = pagos.cliente_id
    {filtro}
    """)

    pagos = cur.fetchall()

    cur.execute(f"""
    SELECT fecha, descripcion, importe, 'GASTO', persona
    FROM gastos
    {filtro}
    """)

    gastos = cur.fetchall()

    datos = pagos + gastos
    datos.sort(key=lambda x: x[0], reverse=True)

    con.close()

    return render_template("informe.html", datos=datos)

# ===============================
# SALDAR
# ===============================
@app.route("/saldar")
def saldar():
    con = conectar()
    cur = con.cursor()

    cur.execute("UPDATE pagos SET saldado = 1 WHERE saldado = 0")
    cur.execute("UPDATE gastos SET saldado = 1 WHERE saldado = 0")

    con.commit()
    con.close()

    return redirect("/")

# ===============================
# ELIMINAR
# ===============================
@app.route("/eliminar_cliente/<int:id>")
def eliminar_cliente(id):
    con = conectar()
    cur = con.cursor()

    cur.execute("DELETE FROM pagos WHERE cliente_id = %s", (id,))
    cur.execute("DELETE FROM clientes WHERE id = %s", (id,))

    con.commit()
    con.close()

    return redirect("/clientes")

@app.route("/eliminar_pago/<int:id>")
def eliminar_pago(id):
    con = conectar()
    cur = con.cursor()

    cur.execute("DELETE FROM pagos WHERE id = %s", (id,))
    con.commit()
    con.close()

    return redirect("/cobros")

@app.route("/eliminar_gasto/<int:id>")
def eliminar_gasto(id):
    con = conectar()
    cur = con.cursor()

    cur.execute("DELETE FROM gastos WHERE id = %s", (id,))
    con.commit()
    con.close()

    return redirect("/gastos")

# ===============================
# MAIN
# ===============================
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))
    app.run(host="0.0.0.0", port=port)